...
def process_client_dbstatline(thisline,fromtime,totime,thislist,granularity,exclusions,loghost,clusterurl,\
...
def find_client_dbstats(logfile,fromtime,totime,granularity,exclusions,loghost,cluster_url):
   ...
   if os.path.isfile(logfile):
     ...
     while ((line != '') and not endlinefound):
      linetime = process_client_dbstatline(line,fromtime,totime,linedbstatlist,granularity,exclusions,loghost,cluster_url,\
        linetime_format,linetime_index,linetime_start,linetime_end,timings_index,status_index,size_index,base_index)
      if not startlinefound:
       if int(linetime) >= int(fromtime):
        startlinefound = True
        logging.warn('{cloudant client data collector} Start of time boundary detected <' + str(fromtime) + '> at line <'+str(linecount)+'>')
      if int(linetime) >= int(totime):
        endlinefound = True
        logging.warn('{cloudant client data collector} End of time boundary detected <' + str(totime) + '> at line <'+str(linecount)+'>')
      line = lf.readline()
      linecount = linecount+1
     if not startlinefound:
      print('{cloudant client data collector} No lines found for selected collection period starting <' + str(fromtime) + '>')
      logging.warn('{cloudant client data collector} No lines found for selected collection period starting <' + str(fromtime) + '>')
     lf.close()
   return pd.DataFrame(linedbstatlist,columns=['cluster','loghost','client','ltime','mtime', 'mtime_epoch','verb','tq','tr','tt','ttr','status','size','endpoint'])
...
def execute_client_collect(scope,s_url,s_credentials,s_username,s_password,p_url,certverif,inputlogfile,thresholdsfile,\
   ...
   try:   
      ...
      event_stats = [] 
      numstats = 0
      basestats = find_client_dbstats(inputlogfile,fromtime,totime,granularity,read_exclusions(statsexclusionsfile),loghost,cluster_url)
