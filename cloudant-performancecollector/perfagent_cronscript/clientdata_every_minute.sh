...
logfile=`echo "/opt/cloudant-performancecollector/perfagent_cronscript/"client$4"_"$1"_"$frompretty"_haproxy.log"`
...
/usr/bin/python /opt/cloudant-performancecollector/clientdata_collect.py -O csv -s $1 -g minute -f $frompretty -t $topretty -L $logfile -H $4 -x $conninfo
...
psqlfile=`echo "/opt/cloudant-performancecollector/perfagent_cronscript/"client$4"_"$1"_"$frompretty"runner.sql"`
if [ $1 == "all" ]; then
