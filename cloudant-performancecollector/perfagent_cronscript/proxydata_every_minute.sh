...
logfile=`echo "/opt/cloudant-performancecollector/perfagent_cronscript/"$HOSTNAME"_"$1"_"$frompretty"_haproxy.log"`
...
/usr/bin/python /opt/cloudant-performancecollector/perfagent.py -O csv -s $1 -g minute -f $frompretty -t $topretty -L $logfile -H $HOSTNAME -x $conninfo
...
psqlfile=`echo "/opt/cloudant-performancecollector/perfagent_cronscript/"$HOSTNAME"_"$1"_"$frompretty"runner.sql"`
if [ $1 == "all" ]; then
